...
import { getServerLatency, measureLatency, TRACE_SERVERS } from './services/api';
...
function App() {
  ...
  const simulateTrace = async (host: string, server: TraceServer) => {
    const hops: TraceResult[] = [];
    const numHops = Math.floor(Math.random() * 5) + 8; // 8-12 hops
    ...
    for (let i = 1; i <= numHops; i++) {
      ...
      hops.push({
        hop: i,
        ip: `192.0.2.${i}`, // Example IP from TEST-NET-1
  ...
  const handleTrace = async (e: React.FormEvent) => {
    ...
    try {
      const results = await Promise.all(
        TRACE_SERVERS.map(server => simulateTrace(target, server))
      );
  ...
  const exportData = () => {
    ...
    const csv = traceResults.flatMap(result => {
      ...
      const headers = ['Hop', 'IP', 'Location', 'Latency (ms)', 'Provider'].join(',');
      const rows = result.hops.map(hop => 
        [hop.hop, hop.ip, hop.location, hop.latency, hop.provider].join(',')
      );
                ...
                <div key={result.sourceServer.id} className="border-b border-gray-200 last:border-b-0">
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {result.hops.map((hop) => (
                        <tr key={hop.hop}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{hop.hop}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{hop.ip}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{hop.location}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{hop.latency.toFixed(2)} ms</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{hop.provider}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
